...
vi.mock('#/api/gen/info', () => ({
  getInfoPage: vi.fn().mockResolvedValue({ list: [], total: 0 }),
  createInfo: vi.fn().mockResolvedValue({}),
  updateInfo: vi.fn().mockResolvedValue({}),
  deleteInfo: vi.fn().mockResolvedValue({}),
}));


describe('GenInfoForm', () => {
  it('can be imported', async () => {
    const mod = await import('#/views/gen/info/form.vue');
