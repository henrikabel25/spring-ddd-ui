...
vi.mock('#/api/sys/user', () => ({
  getUserPage: vi.fn().mockResolvedValue({ list: [], total: 0 }),
  delUserById: vi.fn().mockResolvedValue({}),
}));
...
vi.mock('./form.vue', () => ({
  default: { name: 'UserForm', template: '<div>form</div>' },
}));

vi.mock('./link.vue', () => ({
  default: { name: 'LinkForm', template: '<div>link</div>' },
}));

vi.mock('./recycle.vue', () => ({
  default: { name: 'RecycleForm', template: '<div>recycle</div>' },
}));
...
describe('UserIndex', () => {
  ...
  it('can be imported', async () => {
    const mod = await import('#/views/sys/user/index.vue');
