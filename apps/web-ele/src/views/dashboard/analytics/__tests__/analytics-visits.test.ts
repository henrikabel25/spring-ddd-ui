...
vi.mock('@vben/plugins/echarts', () => ({
  EchartsUI: { name: 'EchartsUI', template: '<div>chart</div>' },
  useEcharts: vi.fn().mockReturnValue({ renderEcharts: vi.fn() }),
}));

describe('AnalyticsVisits', () => {
  it('can be imported', async () => {
    const mod = await import('#/views/dashboard/analytics/analytics-visits.vue');
